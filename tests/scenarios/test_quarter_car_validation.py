...
def test_phase_1_2_runner_writes_acceptance_artifacts(tmp_path: Path, repo_root: Path) -> None:
    ...
    assert (run_dir / "figures" / "free_vibration.pdf").exists()
    assert (run_dir / "figures" / "transmissibility.pdf").exists()
    assert (run_dir / "figures" / "bump_tire_force.pdf").exists()
    assert (run_dir / "figures" / "chirp_response.pdf").exists()
